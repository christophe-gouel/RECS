...
case 'f';
	n = size(s,1);
        ...
        if output.F
		out1 = zeros(n,2);
		out1(:,1) = -1 + (-x(:,1) + s(:,2)).^(-1 + p(4)).*x(:,1).^p(2).*p(1).*p(4).*z(:,1);
		out1(:,2) = -x(:,2) + p(1).*z(:,2) + x(:,1).^(1 - p(2))./(1 - p(2));
		...
		out2(:,1,2) = -(-x(:,1) + s(:,2)).^(-1 + p(4)).*x(:,1).^p(2).*(1 - p(4)).*p(1).*p(4).*z(:,1)./(-x(:,1) + s(:,2)); % d eq_1 w.r.t. w
        ...
        if output.Jx
		out3 = zeros(n,2,2);
		out3(:,1,1) = (-x(:,1) + s(:,2)).^(-1 + p(4)).*x(:,1).^p(2).*(1 - p(4)).*p(1).*p(4).*z(:,1)./(-x(:,1) + s(:,2)) + (-x(:,1) + s(:,2)).^(-1 + p(4)).*x(:,1).^p(2).*p(1).*p(2).*p(4).*z(:,1)./x(:,1); % d eq_1 w.r.t. c
		out3(:,1,2) = 0; % d eq_1 w.r.t. Welf
		out3(:,2,1) = x(:,1).^(1 - p(2))./x(:,1); % d eq_2 w.r.t. c
		out3(:,2,2) = -1; % d eq_2 w.r.t. Welf

        end
        % df/dz
        if output.Jz
		out4 = zeros(n,2,2);
		out4(:,1,1) = (-x(:,1) + s(:,2)).^(-1 + p(4)).*x(:,1).^p(2).*p(1).*p(4); % d eq_1 w.r.t. eh
		out4(:,1,2) = 0; % d eq_1 w.r.t. eW
		out4(:,2,1) = 0; % d eq_2 w.r.t. eh
		out4(:,2,2) = p(1); % d eq_2 w.r.t. eW
...
case 'g';
	n = size(s,1);
        ...
        if output.Js
		out2 = zeros(n,2,2);
		out2(:,1,1) = p(3); % d eq_1 w.r.t. A
		out2(:,1,2) = 0; % d eq_1 w.r.t. w
		out2(:,2,1) = (-x(:,1) + s(:,2)).^p(4).*p(3); % d eq_2 w.r.t. A
		out2(:,2,2) = (-x(:,1) + s(:,2)).^p(4).*(1 - (1 - s(:,1)).*p(3) + e(:,1)).*p(4)./(-x(:,1) + s(:,2)); % d eq_2 w.r.t. w
		...
		out3(:,1,2) = 0; % d eq_1 w.r.t. Welf
		out3(:,2,1) = -(-x(:,1) + s(:,2)).^p(4).*(1 - (1 - s(:,1)).*p(3) + e(:,1)).*p(4)./(-x(:,1) + s(:,2)); % d eq_2 w.r.t. c
