...
\subsection{GRO2: Stochastic growth model with irreversible investment}
\label{sec:gro1}
...
\begin{equation}
  C_{t}: C_{t}\le a e^{Z_{t}}K_{t}^{\alpha} \CP C_{t}^{-\tau}\le\beta\E_{t}\left[C_{t+1}^{-\tau}\left(1-\delta+a \alpha e^{Z_{t+1}}K_{t+1}^{\alpha-1}\right)\right].
\end{equation}
